const loadPost = async (search) => {
  ...
  if (search === undefined){
    ...
    displayPost(posts,search)
  }
};

const displayPost = (posts,search) => {
  ...
  if(search === undefined){    
    posts.forEach((post) => {   
      ...
      <div class="mx-4 pb-4 post-info flex flex-col gap-1 w-full">
        ...
        <div class="flex justify-between border-t border-dashed border-[#12132D40] pt-2 ">                
          <div class="like-cmnt flex gap-2 md:gap-4">
            <div class="comment flex items-center gap-1 text-[rgba(18,19,45,0.6)]">
              <img class="h-4" src="./images/comment.png" alt="">
              <p class="text-xs md:text-sm">${post["comment_count"]}</p>
            </div>
  
            <div class="comment flex items-center gap-1 text-[rgba(18,19,45,0.6)]">
              <img class="h-4" src="./images/view.png" alt="">
              <p class="text-xs md:text-sm">${post["view_count"]}</p>
            </div>
  
            <div class="comment flex items-center gap-1 text-[rgba(18,19,45,0.6)]">
              <img class="h-4" src="./images/timer.png" alt="">
              <p class="text-xs md:text-sm">${post["posted_time"]} min</p>
            </div>
          </div>
    ...
    posts.forEach((post) => {
      if(search === post["category"].toLowerCase() || search === ""){
        const postCard = document.createElement("div");
        postCard.classList =
          "single-card py-6 rounded-lg w-full bg-[#F3F3F5] flex justify-between font-inter";
        postCard.innerHTML = `
        ...
        <div class="mx-4 pb-4 post-info flex flex-col gap-1 w-full">
          ...
          <!-- like-comment -->
          <div class="flex justify-between border-t border-dashed border-[#12132D40] pt-2 text-sm md:text-base">                
            <div class="like-cmnt flex gap-2 md:gap-4">
              <div class="comment flex items-center gap-1 text-[rgba(18,19,45,0.6)]">
                <img class="h-4" src="./images/comment.png" alt="">
                <p class="text-xs md:text-sm">${post["comment_count"]}</p>
